...
public class Env extends Environment {
    ...
    /** creates the agents percepts based on the Model */
    void updatePercepts() {
        clearPercepts("car");
		clearPercepts("motor");
		...
		if (lBicycle.equals(model.lMydepotbic)) {
    ...
    public boolean executeAction(String ag, Structure action) {
        ...
        if (action.equals(literalCARopensItsDepot)) {
            result = model.openMydepot();
        }
		else if (action.equals(literalCARclosesItsDepot)) {
		...
		else if (action.getFunctor().equals("move_towards_motor")) {
            String l = action.getTerm(0).toString();
            Location dest = null;
		...
		else if (action.getFunctor().equals("move_towards_bic")) {
            String l = action.getTerm(0).toString();
            Location dest = null;
            ...
            try {
				...
				result = model.addJuice( (int)((NumberTerm)action.getTerm(1)).solve());
